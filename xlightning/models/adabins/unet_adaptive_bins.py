...
class DecoderBN(nn.Module):
    def __init__(self, num_features=2048, num_classes=1, bottleneck_features=5504): # original bottleneck_features = 2048 | 5504
        ...
        self.up1 = UpSampleBN(skip_input=int(num_features)+480, output_features=features // 2)
        #self.up2 = UpSampleBN(skip_input=features // 2 + 40 + 24, output_features=features // 4)
        self.up2 = UpSampleBN(skip_input=1200, output_features=features // 4)
        #self.up3 = UpSampleBN(skip_input=features // 4 + 24 + 16, output_features=features // 8)
        self.up3 = UpSampleBN(skip_input=616, output_features=features // 8)
        #self.up4 = UpSampleBN(skip_input=features // 8 + 16 + 8, output_features=features // 16)
        self.up4 = UpSampleBN(skip_input=328, output_features=features // 16)
...
class UnetAdaptiveBins(nn.Module):
    ...
    def build(cls, n_bins, **kwargs):
        ...
        basemodel_name = 'tf_efficientnet_l2_ns'

        print('Loading base model ()...'.format(basemodel_name), end='')
        basemodel = torch.hub.load('rwightman/gen-efficientnet-pytorch', basemodel_name, pretrained=True)#torch.hub.load('/petrobr/home/robinson.garcia/.cache/torch/hub/rwightman_gen-efficientnet-pytorch_master', basemodel_name, pretrained=True,source='local')
