...
class local_planar_guidance(nn.Module):
    def __init__(self, upratio,device='cuda:2'):
        super(local_planar_guidance, self).__init__()
        self.upratio = torch.tensor(upratio)
        self.u = torch.arange(self.upratio).reshape([1, 1, self.upratio])
        self.v = torch.arange(int(self.upratio)).reshape([1, self.upratio, 1])
        #self.upratio = float(upratio)
        self.device=device

    def forward(self, plane_eq, focal):
        focal = focal.type_as(plane_eq)
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels,num_features=512,device=None):
        ...
        self.lpg8x8     = local_planar_guidance(8,device=device)
        ...
        self.lpg4x4     = local_planar_guidance(4,device=device)
        ...
        self.lpg2x2     = local_planar_guidance(2,device=device)
    ...
    def forward(self, features, focal):
        ...
        depth_8x8_scaled_ds = torch_nn_func.interpolate(depth_8x8_scaled, scale_factor=0.25, mode='bilinear',align_corners=True)
        ...
        depth_4x4_scaled_ds = torch_nn_func.interpolate(depth_4x4_scaled, scale_factor=0.5, mode='bilinear',align_corners=True)
...
class BtsModel(nn.Module):
    def __init__(self, params,device):
        ...
        self.decoder = bts(params, self.encoder.feat_out_channels, params.bts_size,device=device)
